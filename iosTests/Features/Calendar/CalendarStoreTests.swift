...
struct CalendarStoreTests {
  ...
  @Test func days_from_count_and_event_distribution() async throws {
    ...
    _ = try await store.addEntry(
      title: "Span", startDate: makeDate(2025, 4, 1, 23, 0), endDate: makeDate(2025, 4, 2, 1, 0))
    _ = try await store.addEntry(
      title: "D1Only", startDate: makeDate(2025, 4, 1, 10, 0), endDate: makeDate(2025, 4, 1, 11, 0))
  ...
  @Test func months_from_skips_past_days_in_current_month_and_handles_rollover() async throws {
    ...
    _ = try await store.addEntry(
      title: "Past", startDate: makeDate(2025, 5, 5, 9, 0), endDate: makeDate(2025, 5, 5, 10, 0))
    _ = try await store.addEntry(
      title: "Future", startDate: makeDate(2025, 5, 20, 9, 0), endDate: makeDate(2025, 5, 20, 10, 0)
    )
